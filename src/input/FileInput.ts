...
import { Database, RecordReader, UdbRecord } from "./db"
...
export class FileInput implements Input {
  ...
  hasNext(): boolean {
    const has = this.filePos + this.recordSize < this.fileSize
  ...
  readRecord(recordIndex: number): Promise<UdbRecord> {
    ...
    return new Promise((resolve, reject) => {
      const bytesRead = fs.readSync(this.fd, this.buffer, 0, this.recordSize, this.filePos)
      const inputRecord = this.recordReader.read(this.filePos, recordIndex)
